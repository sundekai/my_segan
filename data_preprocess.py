...
clean_train_folder = '/media/sundekai/DATA/1-sundekai/data_final/timit_part/TRAIN/train_clean'
noisy_train_folder = '/media/sundekai/DATA/1-sundekai/data_final/timit_part/TRAIN/train_noisy'
...
def process_and_serialize(data_type):
    ...
    for root, dirs, files in os.walk(clean_folder):
        ...
        for filename in tqdm(files, desc='Serialize and down-sample {} audios'.format(data_type)):
            clean = filename
            noise = filename.replace('clean','mix')
            filename = filename.replace('-clean','')
            clean_file = os.path.join(clean_folder, clean)
            noisy_file = os.path.join(noisy_folder, noise)
